 ...
 * <p>
 * It stores the required node and edge information in different int-arrays.
 * 
...
public class HPListGraph<NodeType, EdgeType> implements
	...
	private ArrayList<NodeType> nodeLabels;
	...
	public ArrayList<NodeType> getNodeLabels(){
		nodeLabels = new ArrayList<>();
		nodeLabels.addAll(Arrays.asList(node_labels));
		while (nodeLabels.contains(null)){
			nodeLabels.remove(null);
		}
		return nodeLabels;
	}
...
//			System.out.println(getDirection(edgeIdx));
	...
	/**Convert a 'directedWeighedMultiGraph' into a HPListGraph. 17 Aug 2018.**/
	public static void loadDirectedWeightedMultiGraph(DirectedWeightedMultigraph<edu.isi.karma.rep.alignment.Node, LabeledLink> directedWeightedMultigraph, HPListGraph hpListGraph){
		...
		while (iterator.hasNext()){
			...
			int index = hpListGraph.addNodeIndex(Integer.valueOf(label));//add a node into the HPList graph, and output an index
		...
		while (iterator1.hasNext()){
			...
			hpListGraph.addEdgeIndex(startId,targetId,String.valueOf(edgeLabel),1);//add this edge into the HPList graph.
		}
	}
