...
//					.getParameterValue(ContextParameter.TEXTUAL_SEMTYPE_MODEL_DIRECTORY);
	...
	public static void main(String[] args) throws IOException {
		...
		HybridSTModelHandler modelHandler = new HybridSTModelHandler("C:\\Users\\Dell\\karma");
		modelHandler.modelEnabled = true;

		modelHandler.removeAllLabels();


		File trainDir = new File("D:\\ASM\\experiment\\exp_20220228\\trainsColumns");
		File testDir = new File("D:\\ASM\\experiment\\exp_20220228\\newColumns");

		FileWriter fw = new FileWriter("D:\\ASM\\experiment\\exp_20220228\\result.txt");

		for (File trainFile : Objects.requireNonNull(trainDir.listFiles())) {
			String label = trainFile.getName().substring(4, trainFile.getName().indexOf('.'));
			List<String> trainExamples = new ArrayList<>();
			Scanner scanner = new Scanner(trainFile);
			while (scanner.hasNext()){
				trainExamples.add(scanner.next());
			}
			modelHandler.addType(label, trainExamples);
		}

		for (File testFile : Objects.requireNonNull(testDir.listFiles())) {
			List<String> testExamples = new ArrayList<>();
			Scanner scanner = new Scanner(testFile);
			while (scanner.hasNext()){
				testExamples.add(scanner.next());
			}
			List<SemanticTypeLabel> predictTypes =  modelHandler.predictType(testExamples, 3);
			System.out.println(testFile.getName());
			fw.write("attribute: " + testFile.getName().substring(0, testFile.getName().indexOf('.')) + "\n");
			for (SemanticTypeLabel predictType : predictTypes) {
				System.out.println(predictType);
				fw.write(predictType.toString()+"\n");
			}
			fw.write("\n");
			System.out.println();
		}
