...
var applyModelDialog = (function() {
	...
	function PrivateConstructor() {
		...
		function saveDialog(e) {
			...
			if (modelExist) {
				console.log("Zaiwen telll you, model exists here or not? " + modelExist);
			...
			console.log("begin...");//just for test. 4/2/18
			console.log(info["worksheetId"]);

			//showLoading(info["worksheetId"]);//removed on 4/2/2018
            console.log("end...");
			var returned = sendRequest(info, worksheetId);
		};
	...
	function getInstance(wsId) {
		if (!instance) {
			instance = new PrivateConstructor();
			instance.init();  //initialize the 'Apply Model From Repository' dialog. 3/2/18
		}
...
var PublishJSONDialog = (function() {
	...
	function PrivateConstructor() {
		...
		function saveDialog(e, importAsWorksheet) {
			...
			showLoading(info["worksheetId"]);
            console.log("Now I am ready to generage Json...")
			var returned = $.ajax({
				url: "RequestController",
				type: "POST",
				data: info,
...
//add a new diagram for filling in the information of writing mapping rules to CDM mapping repository. Edited on 28 Jan 2018.
var WriteCDMDialog = (function() {
    // var instance = null;
    //
    // function PrivateConstructor() {
    //     var dialog = $("#writeMappingToCDM");
    //     var worksheetId;
    //     var availableModels;
    //     var filteredModels;
    //     var table;
    ...
    //     function init() {
    //
    //     	//Code below is added on 3/2/2018
    //         var dialogContent = $("#applyModelDialogColumns", dialog);
    //         table = $("<table>")
    //             .addClass("table table-striped table-condensed");
    //         var tr = getHeaderRow();
    //         table.append(tr);
    //         dialogContent.append(table);
		// 	//Code above is added on 3/2/2018
    //
    //         //Initialize what happens when we show the dialog
    //         dialog.on('show.bs.modal', function(e) {
    //             hideError();
    ...
    function PrivateConstructor() {
        ...
        function init() {
            ...
            table.append(tr);
            dialogContent.append(table);
            $('#btnSave1', dialog).on('click', function(e) {
                e.preventDefault();
                saveDialog(e);
            });
        }
