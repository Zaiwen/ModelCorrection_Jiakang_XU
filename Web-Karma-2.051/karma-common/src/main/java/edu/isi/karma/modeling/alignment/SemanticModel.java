...
//		}
	...
	/**This function writes a semantic model into JSON file. 1 June 2018.**/
...
//		else GraphUtil.writeGraph(workspace, worksheet, GraphUtil.asDefaultGraph(this.graph), writer);
	...
	/**read the actual json file of semantic model (e.g. ile_ds1.model.json). 4 JUNE 2018.**/
	...
	/**Get all of the object property links of the semantic model. 15 Aug 2018**/
	public Set<ObjectPropertyLink> getObjectPropertyLink() {
	    Set<ObjectPropertyLink> objectPropertyLinks = new HashSet<>();
	    if (this.graph != null) {
            for (LabeledLink l : this.graph.edgeSet())
                if (l instanceof ObjectPropertyLink)
                    objectPropertyLinks.add((ObjectPropertyLink) l);
        }
        return objectPropertyLinks;
    }


	/**Get a clean graph which only contains internal nodes and object linking edges
	 * @from 15 Aug 2018**/
	public DirectedWeightedMultigraph<Node,LabeledLink> getSimpliedGraph(){
	    DirectedWeightedMultigraph<Node,LabeledLink> simpleGraph = new DirectedWeightedMultigraph<>(LabeledLink.class);
        Set<InternalNode> internalNodes = this.getInternalNodes();//get all the internal nodes
        Set<ObjectPropertyLink> objectPropertyLinks = this.getObjectPropertyLink();//get all the object property links

        Iterator<InternalNode> iterator = internalNodes.iterator();
        while (iterator.hasNext()){
            InternalNode node = iterator.next();
            simpleGraph.addVertex(node);
        }

        Iterator<ObjectPropertyLink> iterator1 = objectPropertyLinks.iterator();
        while (iterator1.hasNext()){
            ObjectPropertyLink objectPropertyLink = iterator1.next();
            LabeledLink copiedLink = objectPropertyLink.clone();//copy the link
            simpleGraph.addEdge(objectPropertyLink.getSource(),objectPropertyLink.getTarget(),copiedLink);
        }
        return simpleGraph;
    }
}
