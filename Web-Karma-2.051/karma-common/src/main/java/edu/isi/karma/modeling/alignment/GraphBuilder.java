...
//			logger.debug("time to update links to Thing (root): " + elapsedTimeSec);
	...
	/**Adding paths from domain ontology to an alignment graph. 2 June 2019.**/
	public void addClosureAndUpdateLinks(Set<InternalNode> internalNodes, Set<Node> addedNodes) {
		...
		if (internalNodes != null) {
			...
			for (Node node : nodes)
				//if (this.idToNodeMap.containsKey(node.getId())) //remove on 3 June 2019.
					addNodeClosure(node, addedNodes);
		}
		...
		updateLinks();/**all all the paths from domain ontology. 3 June 2019.**/
...
//		if (link instanceof DataPropertyLink) return;
	...
	/**Very important. 2 June 2019. Compute all the dependents of a node*
	 * @param uri a node
	 * @return all the dependents
	 * */
	private Set<String> computeUriClosure(String uri) {
		...
		HashMap<String, Set<String>> dependentUrisMap = new HashMap<>();//initialize dependent nodes map. 2 June 2019.
	...
	/**add all the paths from domain ontology. 3 June 2019.**/
