...
public class RecursiveStrategy<NodeType, EdgeType> implements
	...
	public Collection<Fragment<NodeType, EdgeType>> search(final Algorithm<NodeType, EdgeType> algo) {
		...
		for (final Iterator<SearchLatticeNode<NodeType, EdgeType>> it = algo.initialNodes(); it.hasNext();) {
			...
			/**I guess that this code is the initial edge when we begin to **/
			...
			//we delete edges in graphs that contain the edge that the code stands for and only look for frequent patterns in what is left
			it.remove();

			if (VERBOSE) {
				out.println("\tdone (" + (System.currentTimeMillis() - time) + " ms)");
			}
		}
