...
public class Parser {
	...
	public MappingDocument parse(boolean skipDatabaseConnection) {

		init(skipDatabaseConnection);
		...
		try {
			...
			LinkedList<LogicalTableMapping> logicalTableMappings = findLogicalTableMappings(skipDatabaseConnection);
	...
	public LinkedList<LogicalTableMapping> findLogicalTableMappings(boolean skipDatabaseConnection) {
			    	...
			    	if (rn.asResource().getURI() != null) {
			    		...
			    		if (sparqlResults.getRows().size() > 0) {
			    			...
			    			if (sparqlResults.getRows().size() > 0) {
			    				...
			    				logicalTableMapping.setView(logicalTableView);
			    				if (!contains(results, logicalTableMapping.getUri())) {
			    					logicalTableMapping.setDatabseTableName(tableName);
					    			results.add(logicalTableMapping);
			    				} else if(logicalTableMapping.getDatabseTableName() != null){ //Set the database table name so that we can use it later on
			    					getLogicalTableMapping(results, logicalTableMapping.getUri()).setDatabseTableName(tableName);
			    				}
			    			}
			    		}
			    	}
		...
		while (iter2.hasNext()) {
		    ...
		    while (iter2b.hasNext()) { //should be only 1
		    	...
		    	if (!contains(results, logicalTableMapping.getUri())) {
		    		logicalTableMapping.setDatabseTableName(newTable);
		    		results.add(logicalTableMapping);
		    	} else if(logicalTableMapping.getDatabseTableName() != null) { //Set the database table name so that we can use it later on
		    		getLogicalTableMapping(results, logicalTableMapping.getUri()).setDatabseTableName(newTable);
				}
		    }
		}
	...
	public String createQueryForTable(String tableName) {
		...
		try {
            ...
            } else if (mappingDocument.getDatabaseType() == DatabaseType.SQLSERVER) {
            	//Remove the schema name
            	if(tableName.indexOf(".") != -1) {
            		tableName = tableName.substring(tableName.indexOf(".") + 1);
            		log.info("SQL Server table name without schema: " + tableName);
            	}
            	
            	rs = stmt.executeQuery("select Column_name from Information_schema.columns where Table_name like '" + tableName + "'");
            } else {
                rs = null;
			...
			String field = null;
			log.info("Database fields are: ");
			
			rs.beforeFirst();
			while (rs.next()) {
				
				//mysql: fields.add(rs.getString("Field"));
				if (mappingDocument.getDatabaseType() == DatabaseType.POSTGRESQL || mappingDocument.getDatabaseType() == DatabaseType.ORACLE || mappingDocument.getDatabaseType() == DatabaseType.SQLSERVER) {
				...
				log.info("Field "+ resultSetCount +":" + field);
			}
			
			log.info("resultSetCount: " + resultSetCount);
	...
	public void init(boolean skipDatabaseConnection) {
	...
	boolean contains(LinkedList<LogicalTableMapping> logicalTableMappings, String uri) {
		...
		if(getLogicalTableMapping(logicalTableMappings, uri) != null) {
			return true;
		}
	...
	LogicalTableMapping getLogicalTableMapping(LinkedList<LogicalTableMapping> logicalTableMappings, String uri) {
		
		for (LogicalTableMapping logicalTableMapping : logicalTableMappings) {
			if (StringUtils.equals(logicalTableMapping.getUri(), uri)) {
