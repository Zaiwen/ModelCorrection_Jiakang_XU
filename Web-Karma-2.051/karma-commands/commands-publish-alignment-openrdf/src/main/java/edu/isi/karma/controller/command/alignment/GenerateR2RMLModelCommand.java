...
import gr.seab.r2rml.util.CDMMapping;
import gr.seab.r2rml.util.R2RMLUtil;
...
public class GenerateR2RMLModelCommand extends WorksheetSelectionCommand {
	...
	public UpdateContainer doIt(Workspace workspace) throws CommandException {
		...
		try {
			...
			/**Write R2RML JSON file into disk. by Zaiwen on 5 Feb 2018**/
			...
			if (tripleStoreUrl != null && tripleStoreUrl.trim().compareTo("") != 0) {
				try {
					...
					/**this url is the address of a newly generated r2rml file on the Mac. 5 Feb 2018.**/
					...
					//Here we call the R2RML parser. 2018.02
					...
					LinkedList<LogicalTableMapping> logicalTableMappings = R2RMLUtil.parseR2RML(url,r2rmlPropFileAbsPath,inputModelPath,applicationContext);
					List<CDMMapping> cdmMappings = R2RMLUtil.normalise(logicalTableMappings);//normalize

                    List<String> tableType = new ArrayList<>();//added 2019.03.27

					//Here upload the mapping rules into MongoDB. 2019.03.20
                    org.json.simple.JSONObject cdmMappingJsonStr_Forward = R2RMLUtil.createJSON_Forward(cdmMappings,owlFilePath, tableType);//create JSON object
					logger.info("Mapping rules JSON file has already been generated. " + cdmMappingJsonStr_Forward);

                    logger.info("Now begin to upload this JSON object to MangoDB!");
                    MongoClient client = MongoFunctions.getClient("mongodb://MRuser:unisa@103.61.226.39:27017/MR");

                    //upload to the different types of collections
                    String collectionName = "";
                    if (tableType.get(0).equals("GENERAL")) {
                        //collectionName = "Zaiwen.attributeMapping.rules";
						collectionName = "Zaiwen";
                    } else if (tableType.get(0).equals("OBJECTLINK")) {
                        //collectionName = "Zaiwen.relationshipMapping.rules";
						collectionName = "Zaiwen.relationshipMapping";
                    }

                    MongoCollection<Document> collection = MongoFunctions.getCollection("MR", collectionName, client);
                    boolean mongoCall = MongoFunctions.insertDocument(cdmMappingJsonStr_Forward, "MR", collectionName, client);
                    System.out.println("The http response code for uploading forward mapping from CDM to DS is: " + mongoCall);
                    if (mongoCall) {
                        logger.info("JSON object has been successfully uploaded!");
                        logger.info("All the documents in this collection is: " + collection.count());
                    } else {
                        logger.info("JSON object has NOT been successfully uploaded!");
                    }
                    client.close();
